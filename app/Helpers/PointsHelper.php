...
class PointsHelper
{
    ...
    // Fetch data from the database and determine how much pointsthe user gets
    ...
    // Determine how many points the user would like to exchange for a discount
    public static function exchangePoints($points_exchanged)
    {
        try {
            // Points excahnged converted to a number
            $points_exchanged = (int) $points_exchanged;

            // Check to see if user has enough points to exchange
            if ($points_exchanged > Auth::user()->total_points) {
                return 'You do not have enough points';
            }

            // Check database for exchanging points
    ...
    {
        $this->discounted_price = CustomHelper::calculateDiscountedPrice($this->total, $this->getPointsDiscountApplied());
